...
class ReadFile:
    ...
    def _resolve_path(path_value, base_path=BASE_PATH):
        """Resolve mixed Windows/Linux separators to an absolute path."""
    ...
    def get_AI_json_stocks(jsonFile= '\\aitrader\\data\\output\\json\\al_agent_stock_program.json'):
        ...
        all_codes = list(dict.fromkeys(
            main_fundFlow__codes +
            main_industry_analysis_codes +
            main_fundamental_analysis_codes +
            longhuban_stocks
        ))
        print(f'合并并去重所有股票代码:{all_codes}')
        return all_codes, al_agent_stocks

    def appendDatasToExcel(datas, filePath='\\data\\input\\myStock\\我的自选.xlsx', remove_duplicates=True):
        """
