...
class SmartMonitorDeepSeek:
    ...
    def __init__(self, api_key: str = "", base_url: str = "", model: str = "deepseek-chat"):
        ...
        """
        runtime_llm_config = get_runtime_llm_config()
        self.api_key = api_key or runtime_llm_config["api_key"]
        self.base_url = base_url or runtime_llm_config["base_url"]
        self.default_model = model or runtime_llm_config["model"] or "deepseek-chat"
        self.headers = {
    ...
    def _normalize_text(text) -> str:
        """Normalize whitespace for stable prompt size logging."""
        if text is None:
            return ""
    ...
    @classmethod
    def _estimate_tokens(cls, text) -> int:
    ...
    @classmethod
    def _summarize_messages(cls, messages: List[Dict]) -> Dict[str, int]:
        """Summarize prompt size for request logging."""
        prompt_text = "\n".join(cls._normalize_text(message.get("content", "")) for message in messages)
        return {
            "message_count": len(messages),
            "chars": len(prompt_text),
            "estimated_tokens": cls._estimate_tokens(prompt_text)
        }
    ...
    def chat_completion(self, messages: List[Dict], model: str = "deepseek-chat",
        ...
        try:
            ...
            self.logger.info(
                "[LLM][smart-monitor] model=%s msgs=%s prompt_chars=%s prompt_est_tokens=%s "
                "completion_chars=%s completion_est_tokens=%s max_tokens=%s usage=%s",
                model_to_use,
                prompt_stats["message_count"],
                prompt_stats["chars"],
                prompt_stats["estimated_tokens"],
                len(self._normalize_text(content)),
...

{
    ...
    "confidence": 0-100,
    "reasoning": "详细的决策理由，包括技术分析、风险评估等，120-180字",
...
"""
        ...
        try:
            response = self.chat_completion(messages, temperature=0.3, max_tokens=900)
